// URSA IPA — v9.4: Редизайн профиля + быстрая загрузка
...
    login_btn: "Войти через Google", // (Текст для <p> над иконками)
    logout_btn: "Выйти",
    guest: "Гость",
    dash: "—",
    badge_free: "Free",
...
    vip_only: "🔒 VIP Only",
    login_btn: "Sign in with", // (Текст для <p> над иконками)
    logout_btn: "Sign out",
    guest: "Guest",
    dash: "—",
    badge_free: "Free",
...
Â   const res = await fetch(SIGNER_API, { method: "POST", body: form });
...
// === Profile Modal (МОДИФИЦИРОВАНО v9.4) ===
window.openSettings = async function openSettings() {
  ...
  // 2. Находим НОВЫЕ элементы управления
  const loginContainer = info.querySelector("#social-login-container");
  const googleLoginBtn = info.querySelector("#google-login-btn"); // Кнопка Google
  const logoutBtn = info.querySelector("#logout-btn"); // Кнопка Выйти

  // 3. Логика отображения (Вход/Выход)
  if (localStorage.getItem("ursa_email")) {
    // === ПОЛЬЗОВАТЕЛЬ ВОШЕЛ ===
    loginContainer.style.display = "none"; // Прячем иконки входа
    logoutBtn.style.display = "block";   // Показываем кнопку Выхода
    logoutBtn.textContent = __t("logout_btn");
    // `ursaAuthAction` универсальная (и для входа, и для выхода)
    logoutBtn.onclick = () => window.ursaAuthAction && window.ursaAuthAction();
  } else {
    // === ПОЛЬЗОВАТЕЛЬ НЕ ВОШЕЛ ===
    loginContainer.style.display = "block"; // Показываем иконки входа
    logoutBtn.style.display = "none";   // Прячем кнопку Выхода
    // Текст для <p> над кнопками
    loginContainer.querySelector("p").textContent = __t("login_btn"); 
    // Вешаем клик ТОЛЬКО на Google (т.к. auth.js только для него)
    googleLoginBtn.onclick = () => window.ursaAuthAction && window.ursaAuthAction();
    // (Кнопки Facebook и Twitter пока ничего не делают)
  }

  // 4. Сертификат (логика та же)
  ...
  // 6. Открываем модалку (логика та же)
...
          ${__t("load_error")}
Â         <br><small style="opacity:0.5;font-size:12px;">(Нужен индекс, см. консоль F12)</small>
         </div>`;
    } finally {
      state.loading = false;
    }
